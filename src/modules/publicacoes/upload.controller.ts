...
@ApiBearerAuth()
@NivelMinimo('ADMIN')
@Controller('comunicacao/upload')
@ApiTags('Comunicação — Upload')
export class UploadController {
  ...
  @UseInterceptors(
    FileInterceptor('arquivo', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads', 'comunicacao'),
      ...
      fileFilter: (_req, file, cb) => {
        const permitidos = /jpeg|jpg|png|webp|gif|mp4|webm/;
